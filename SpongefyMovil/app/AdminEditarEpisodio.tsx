...
export default function AdminEditarCancion() {
  ...
  const handleActualizar = async () => {
    if (!id || !titulo || !fecha || !descripcion || !creador) {
    ...
    if (id_podcast) formData.append("id_podcast", id_podcast);
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert("Éxito", data.message || "Episodio actualizado con éxito");
        router.back();
      } else {
  ...
  return (
    <View style={styles.container}>
      <ScrollView>
        <Text style={styles.title}>Editar Episodio</Text>
